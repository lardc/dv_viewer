...
{
    public class DataProvider : IFilteredSortedSourceProviderAsync
    {
        public DataProvider(CacheEdit cacheEditHandler, CacheBuildCompleted cacheBuildCompletedHandler, PortionDataLoaded portionDataLoadedHandler)
        {
        ...
        public void Init(BuildColumnInDataGrid buildColumnInDataGridHandler, RemoveXMLDataGridColumns removeXMLDataGridColumnsHandler)
        {
            //запоминаем ссылки на механизмы обратных вызовов
            this.FBuildColumnInDataGridHandler = buildColumnInDataGridHandler;
            this.FRemoveXMLDataGridColumnsHandler = removeXMLDataGridColumnsHandler;
        }
        ...
        private BuildColumnInDataGrid FBuildColumnInDataGridHandler = null;
        private RemoveXMLDataGridColumns FRemoveXMLDataGridColumnsHandler = null;
        ...
        private void FillPortionData(System.Collections.IList listOfItems, System.Data.SqlClient.SqlDataReader reader)
        {
            ...
            Routines.ValuesToRow(reader, item, false, this.FBuildColumnInDataGridHandler);
            listOfItems.Add(item);
        }

        private void ReloadPortionData(System.Collections.IList listOfItems, int index, System.Data.SqlClient.SqlDataReader reader)
        {
            //перечитываем данные из базы данных в запрошенный набор данных
            var item = listOfItems[index];

            if (item is DynamicObj)
                Routines.ValuesToRow(reader, (DynamicObj)item, true, this.FBuildColumnInDataGridHandler);
        }
        ...
        public List<DynamicObj> GetItemsAt(int offSet, int portionSize)
        {
            ...
            this.FRemoveXMLDataGridColumnsHandler?.Invoke();

            lock (this)
            {
                //загрузка запрошенного списка изделий
                SCME.Types.DbRoutines.CacheReadData(this.FillPortionData, null, result, offSet, portionSize, Common.Constants.cString_AggDelimeter);
            }
        ...
        public int Count()
        {
            ...
            lock (this)
            {
                //получаем количество групп изделий кеша
                result = SCME.Types.DbRoutines.CacheBuild(true, Common.Constants.cString_AggDelimeter);
